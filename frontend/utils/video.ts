...
async function uploadVideo(userId: string, file: File, caption: string, canister) {
  const videoBuffer = (await file?.arrayBuffer()) || new ArrayBuffer(0);

  const videoInit = getVideoInit(userId, file, caption);
  const videoId = await canister.createVideo(videoInit);
...
export function useUploadVideo({ userId }: { userId: string }) {
  ...
  const [canister] = useCanister("delivery")
